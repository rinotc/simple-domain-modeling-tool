...
export class BoundedContextUpdatePageComponent implements OnInit {
  ...
  get boundedContextNameErrorMessage(): string {
    requirement(
      this.hasBoundedContextNameValidationError,
      'this method must only call bounded context name has validation error'
    );
  ...
  get boundedContextAliasErrorMessage(): string {
    requirement(
      this.hasBoundedContextAliasValidationError,
      'this method must only call bounded context alias has validation error'
    );
  ...
  get boundedContextOverviewErrorMessage(): string {
    requirement(
      this.hasBoundedContextOverviewValidationError,
      'this method must only call bounded context overview has validation error'
    );
  ...
  get canSubmit(): boolean {
    return (
      BoundedContextAlias.isValid(this.inputBoundedContextAlias) &&
  ...
  onSubmit(): void {
    ...
    const overview = new BoundedContextOverview(
      this.inputBoundedContextOverview
    );

    const updatedBoundedContext = this.targetBoundedContext!.changeAlias(alias)
      .changeName(name)
      .changeOverview(overview);
    console.log('submit');

    this.boundedContextsService.update(updatedBoundedContext).then((r) => {
